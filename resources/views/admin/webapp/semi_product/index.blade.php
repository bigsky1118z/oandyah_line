<x-admin.webapp.frame.basic title="Semi Product" heading="半製品一覧">
<x-slot name="head">
    <style>
        input[type="text"] {
            width: 150px;
...
<form action="/admin/webapp/semi_product" method="get">
        ...
        <tbody>
            ...
            <td>
                <select name="semi_type">
                    <option value="">---</option>
                    <option value="C式[ミ]" @isset($query['semi_type']) @if ($query['semi_type'] == "C式[ミ]") selected @endif @endisset>C式[ミ]</option>
                    <option value="C式[フタ]" @isset($query['semi_type']) @if ($query['semi_type'] == "C式[フタ]") selected @endif @endisset>C式[フタ]</option>
                    <option value="台紙" @isset($query['semi_type']) @if ($query['semi_type'] == "台紙") selected @endif @endisset>台紙</option>
                </select>
            </td>
            <td>
                <select name="cutting">
...
<table>
    ...
    <tbody>
        @if (!$semi_products->isEmpty())
        @foreach ($semi_products as $semi_product)
        <tr>
            <td>{{ $semi_product['code'] }}</td>
            <td>{{ $semi_product->get_display_name() }}</td>
            <td>{{ $semi_product->get_size() }}</td>
            ...
            <td>
                <ul>
                    ...
                    <li>{{ $product->product->code }}</li>
                    @endforeach
                </ul>
            </td>
            <td>
                <ul>
                    <li><a href="/admin/webapp/semi_product/{{ $semi_product['id'] }}">detail</a></li>
                    <li><a href="/admin/webapp/semi_product/{{ $semi_product['id'] }}/edit">edit</a></li>
                </ul>
            </td>
        </tr>
        @endforeach
        @elseif($semi_products->isEmpty())
        <tr><td colspan="14">no semi products</td></tr>
        @endif
    </tbody>
    <tfoot>
        <tr><td colspan="14"><a href="/admin/webapp/semi_product/create">add</a></td></tr>
    </tfoot>
</table>
